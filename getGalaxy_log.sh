...
echo "==================" >> "$1_creating.txt"
counter=1
for pos_name in $(kubectl get pods -a |awk '{if ($3=="ContainerCreating") print $1;}' | grep 'galaxy'); do 
...
counter=1
for pos_name in $(kubectl get pods -a |awk '{if ($3=="Unknown") print $1;}' | grep 'galaxy'); do
...
echo "==================" >> "$1_Error.txt"
counter=1
for pos_name in $(kubectl get pods -a |awk '{if ($3=="Error") print $1;}' | grep 'galaxy'); do
...
#   kubectl get pods -a |awk '{if ($1==$pos_name) print $1,"up time:", $5 ;}' | grep 'galaxy' >> $1
...
counter=1
for pos_name in $(kubectl get pods -a |awk '{if ($3=="Completed") print $1;}' | grep 'galaxy'); do
